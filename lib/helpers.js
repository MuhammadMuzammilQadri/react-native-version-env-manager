...
module.exports = {
  ...
  addPostfixToVersionInPackageInfo(pathToPackage, postFix) {
    let content = fs.readFileSync(pathToPackage, 'utf8');

    let regex = /(\s*"?\s*version\s*"?\s*:\s*"?)([\d.]+)(?:-?([\w.]*))*(?:\+([\w.]+))*(\s*"?\s*)/g;
    let matchedGroups = regex.exec(content);
    let versionCore = matchedGroups[2];
    let build = matchedGroups[4] === undefined ? '' : `+${matchedGroups[4]}`;
    content = content.replace(regex, `$1${versionCore}${postFix}${build}$5`);
    fs.writeFileSync(pathToPackage, content, 'utf8');
  }
};
